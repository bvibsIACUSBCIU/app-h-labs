...
import { Radio, Search, RefreshCw, AlertTriangle, ChevronRight, ExternalLink, Radar, Twitter } from 'lucide-react';
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
                {loading && articles.length === 0 ? (
                    ...
                    filteredArticles.map(article => (
                        <div
                            key={article.id}
                            className={`bg-slate-950/40 border border-white/5 rounded p-3 transition-all hover:bg-slate-900/40 ${expandedId === article.id ? 'border-red-500/20 shadow-lg shadow-red-500/5' : ''}`}
                        >
                            <div className="cursor-pointer" onClick={() => setExpandedId(expandedId === article.id ? null : article.id)}>
                                <div className="flex items-center justify-between mb-1.5">
                                    <span className="text-[10px] font-bold text-slate-500 px-1.5 py-0.5 bg-slate-900 rounded">{article.sourceName}</span>
                                    <span className="text-[10px] font-mono text-slate-600">{formatDate(article.publishedAt)}</span>
                                </div>
                                <h3 className={`text-sm leading-tight font-bold mb-1.5 ${expandedId === article.id ? 'text-white' : 'text-slate-200 line-clamp-2'}`}>{article.title}</h3>
                                {/* 默认显示 2 行预览 */}
                                {!expandedId && (
                                    <p className="text-[11px] text-slate-500 leading-snug line-clamp-2">{article.description}</p>
                                )}
                            </div>
                            {expandedId === article.id && (
                                <div className="mt-2 pt-2 border-t border-white/5 animate-in fade-in duration-300">
                                    <p className="text-xs text-slate-400 leading-relaxed mb-3 line-clamp-8">{article.description}</p>
                                    <a href={article.url} target="_blank" rel="noopener noreferrer" className="text-xs text-red-400 font-bold hover:underline flex items-center gap-1">
